...
export default class Scene {
  ...
  createSkybox() {
    var materialArray = ["RT", "LF", "UP", "DN", "FT", "BK"].map(function(dir) {
      var url = "https://gohtml.ru/assets/skybox" + dir + ".jpg";
  ...
  createCube(params) {
    ...
    if (params.image) {
      var texture;
      texture = new THREE.TextureLoader().load(params.image);
  ...
  updateCameraPosition() {
    ...
    cameraPosition.y += 3;
    var distance = this.camera.position.distanceTo(cameraPosition);

    if (distance < 1) {
      distance = 1;
    }

    var speed = 25 / distance + 1;
